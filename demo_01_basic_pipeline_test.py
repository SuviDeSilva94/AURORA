...
def main():
    ...
    certainty_agent = CertaintyCheckAgent(
        ...
        # GATE 1: Probability Check. If the agent is < 85% certain of the root cause, it aborts.
        # CertaintyCheckAgent needs its own 0.85 baseline because it is a standalone, decoupled micro-service.
        certainty_threshold=0.85,
        # GATE 2: Chaos Check (Variational Free Energy). If executing the fix causes chaos > 2.0, it aborts.
        vfe_threshold=2.0,
    )
    
    # MultiAgentCoordinator needs the 0.85 baseline as a mathematical anchor.
    # The Coordinator implements 'Dynamic Gating'. 
    # CALCULATE DYNAMIC CERTINITY THRESHOLD - If 4 anomalies hit at once, 
    # it dynamically overrides this threshold down to 55% to prevent total system paralysis.
    coordinator = MultiAgentCoordinator(device_id="edge-001", certainty_threshold=0.85)
    ...
    # 3. Inject congestion (synthetic) — use same numeric states as BN (0/1)
